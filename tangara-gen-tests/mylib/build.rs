...
fn main() {
    ...
    std::fs::write("../mylib.json", pkg_json).expect("Error with writing to mylib.json");
    ...
    let p: Package = serde_json::from_str(&std::fs::read_to_string("../mylib.json").unwrap()).unwrap();
